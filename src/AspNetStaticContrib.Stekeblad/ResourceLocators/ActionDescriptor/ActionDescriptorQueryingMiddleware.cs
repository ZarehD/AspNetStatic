...
{
	/// <summary>
	/// Adds an endpoint used by <see cref="ActionDescriptorPageResourceLocator" />
	...
	internal class ActionDescriptorQueryingMiddleware
	{
		...
		{
			if (!context.Request.Path.Equals(Constants.ActionDescriptorRoute))
			{
				// Wrong path, continue like nothing and return
				await _next(context);
				return;
			}
			...
			// Locate and process pages
			var pagesDescriptors = _descriptorCollection.ActionDescriptors.Items
